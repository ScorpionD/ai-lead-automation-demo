...
const technologies = [
  ...
  { name: 'API/Webhooks', note: 'Integration layer · planned', icon: GitBranch },
  { name: 'n8n', note: 'Orchestration · planned', icon: Zap },
  { name: 'OpenAI', note: 'Intelligence · planned', icon: Sparkles },
  { name: 'CRM', note: 'Sales pipeline · planned', icon: Layers3 },
  { name: 'PostgreSQL', note: 'Data storage · planned', icon: Database },
]
...
export default function App() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try { setResult(await leadService.submit(normalizeLead(lead), { simulateError })) }
  ...
  return <>
    ...
    <main id="top" className="page-shell">
      ...
      <div className="demo-banner"><ShieldCheck size={19} /><p><strong>Safe demo mode.</strong> Details stay in your browser. Nothing is stored or sent, and no emails are delivered.</p><span>NO API KEYS</span></div>
        ...
        <section className="form-panel" aria-labelledby="form-title">
          ...
          <form id="lead-form" ref={formRef} onSubmit={submit} noValidate>
            <fieldset disabled={loading}><legend className="sr-only">Lead details</legend>
              ...
              {result && <div className="success-notice" role="status"><Check size={17} />Lead qualified. Your demo assessment is ready.</div>}
              ...
              <div className="form-footnote"><span><ShieldCheck size={14} /> Demo only · no data saved</span><span>* Required fields</span></div>
      ...
      <section className="workflow-section" id="workflow" aria-labelledby="workflow-title"><div className="section-heading"><div><span className="eyebrow">THE BIG PICTURE</span><h2 id="workflow-title">One lead. A connected workflow.</h2></div><span className="outline-tag">Workflow preview</span></div><ol className="workflow-track">{workflow.map((step, index) => <li key={step.title} className={index === 1 ? 'highlight-step' : ''}><div className="step-top"><span className="workflow-icon"><step.icon size={21} /></span><span className="step-number">0{index + 1}</span></div><h3>{step.title}</h3><p>{step.detail}</p><span className="step-status">{index === 0 ? 'Interactive' : index === 1 ? 'Simulated' : 'Planned integration'}</span>{index < 4 && <ChevronRight className="connector" size={18} />}</li>)}</ol><p className="workflow-note">This demo captures and qualifies leads locally. CRM sync, follow-up emails, and manager notifications illustrate the next phase.</p></section>
