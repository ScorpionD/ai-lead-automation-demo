...
code('Validate lead','const body=$json.body;const ai=body?.ai||{};return [{json:{...validate(body),ai:providerConfig({AI_PROVIDER:ai.provider,OPENROUTER_MODEL:ai.model,OPENAI_MODEL:ai.model,ALLOW_PAID_AI:ai.allowPaid===true?"true":"false"})}}];');
...
code('Prepare OpenRouter input','const input=$("Validate lead").first().json;return [{json:openrouterBody(input.lead,input.ai.model)}];');
http('OpenRouter classification','https://openrouter.ai/api/v1/chat/completions','={{ $json }}',[{name:'HTTP-Referer',value:'https://ai-lead-automation-demo.pages.dev'},{name:'X-OpenRouter-Title',value:'AI Lead Automation Demo'}],true,20000);
...
code('Validate OpenRouter output','return [{json:assessment($("Validate lead").first().json.lead,$json,"openrouter")}];');
branch('OpenAI selected','$("Validate lead").first().json.ai.provider === "openai" && !!$env.OPENAI_API_KEY');
code('Prepare AI input','const input=$("Validate lead").first().json;return [{json:openaiBody(input.lead,input.ai.model)}];');
